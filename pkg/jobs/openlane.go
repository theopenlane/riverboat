...
func (c *OpenlaneConfig) getOpenlaneClient() (olclient.Client, error) {
	// validate config
	if err := c.Validate(); err != nil {
		return nil, err
	}

	opts := []openlane.ClientOption{openlane.WithBaseURL(c.OpenlaneAPIHost)}

	opts = append(opts, openlane.WithCredentials(openlane.Authorization{
		BearerToken: c.OpenlaneAPIToken,
	}))

	return olclient.New(opts...)
}
